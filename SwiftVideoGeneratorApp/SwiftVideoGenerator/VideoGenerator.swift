...
public class VideoGenerator: NSObject {
   ...
   */
   // MARK: videoDurations parameter is appliable only to .singleAudioMultipleImage type!!!
    open func generate(withImages _images: [UIImage], videoDurations: [Double] = [], andAudios _audios: [URL], andType _type: VideoGeneratorType, _ progress: @escaping ((Progress) -> Void), outcome: @escaping (Result<URL, Error>) -> Void) {
            ...
            videoWriterInput.requestMediaDataWhenReady(on: media_queue, using: { () -> Void in
              ...
              while (videoWriterInput.isReadyForMoreMediaData && frameCount < numImages) {
                ...
                if VideoGenerator.current.type == .single {
                  ...
                  if VideoGenerator.current.type == .multiple {
                    ...
                    elapsedTime += VideoGenerator.current.audioDurations[frameCount] <= 1 ? VideoGenerator.current.minSingleVideoDuration : VideoGenerator.current.audioDurations[frameCount]
                  } else if VideoGenerator.current.type == .singleAudioMultipleImage {
                      ...
                      if videoDurations.count == 0 {
                          ...
                          elapsedTime += audio_Time * videoDurations[frameCount]
                      }
                  } else {
                    nextStartTimeForFrame = frameCount == 0 ? CMTime(seconds: 0, preferredTimescale: 600) : CMTime(seconds: Double(elapsedTime), preferredTimescale: 600)
  ...
  /// private property to store the different video durations
  fileprivate var videoDurations: [Double] = []
  ...
  /// Private method to check if video durations satisfy requirements
  private func checkVideoDurations() {
